 ...
 *   - Delivery Note (dispatches goods)
 *   - Sales Invoice (bills the customer)
 *
 * Tracks ordered quantities; downstream vouchers reference via linkedVouchers.
 */

export function validate(data) {
  if (!data.partyId) throw ApiError.badRequest('Party (customer) is required for sales order');
...
export function getInventoryEntries() {
  return []; // No inventory on order
}

export function getJournalEntries() {
  return []; // No accounting on order
}
