...
export default function Inventory() {
  ...
  // Modal state
  const [modalType, setModalType] = useState(null); // 'item' | 'group' | 'mc'
  ...
  async function loadAll() {
    setLoading(true);
    try {
      const [itemsRes, groupsRes, mcsRes] = await Promise.all([
        api.get('/items'),
  ...
  async function handleDelete() {
    ...
    try {
      const endpoint = deleteType === 'item' ? `/items/${deleteTarget._id}`
        : deleteType === 'group' ? `/items/groups/${deleteTarget._id}`
        : `/material-centres/${deleteTarget._id}`;
    ...
    { key: 'isDefault', label: 'Default', render: (r) => r.isDefault ? <Badge color="green" size="sm">Yes</Badge> : null },
    {
      ...
      render: (r) => (
        <Group gap={4}>
          ...
          <ActionIcon variant="subtle" color="red" onClick={() => { setDeleteTarget(r); setDeleteType('mc'); }}><IconTrash size={16} /></ActionIcon>
        </Group>
      ),
    },
  ];
